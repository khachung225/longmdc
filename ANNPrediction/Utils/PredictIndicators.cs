...
namespace ANNPrediction.Utils
{
   ...
   public class PredictIndicators
    {
       ...
       private void CreateNetwork(int inputCount, int hiddenLayers, List<int> neuralHidden )
       {
           ...
           for (int i = 0; i < hiddenLayers; i++)
               _network.AddLayer(new BasicLayer(new ActivationSigmoid(), true, neuralHidden[i]));                 /*Hidden layer ActivationTANH  ActivationSigmoid*/
       ...
       public List<PredictionResults> Predict()
       {
           ...
           foreach (var sample in _manager.DataTest)
           {
                   ...
                   result.ActualValue = actualOutput[0] * (_manager.GetMax(_manager.InputSize) - _manager.GetMin(_manager.InputSize)) + _manager.GetMin(_manager.InputSize);
                   result.PredictedValue = predict[0] * (_manager.GetMax(_manager.InputSize) - _manager.GetMin(_manager.InputSize)) + _manager.GetMin(_manager.InputSize);
